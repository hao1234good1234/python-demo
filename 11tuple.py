...
# 1. **命名元组（`collections.namedtuple`）**
# 让元组的字段有名字，像轻量级类！
from collections import namedtuple
Point = namedtuple('Point', ['x', 'y'])
p = Point(3, 4)
print(p.x, p.y)    # 3 4
print(p[0], p[1]) # 3 4（依然支持索引！）
# ✅ 用途：替代简单类，节省内存，代码更清晰。
...
ok, msg = check_user(4)
print(ok, msg)
# Python 社区普遍接受这种“隐式元组返回”风格。

#  4. **元组的哈希性（为什么能做字典键？）**
# - 因为元组 **不可变** → **可哈希（hashable）** → 可做字典键或集合元素
# - 但**包含可变对象的元组不可哈希**！
t1 = (1, 2, 3)
print(hash(t1))  # ✅ 可哈希, 输出：529344067295497451

# t2 = ([1, 2], 3)
# print(hash(t2))  # ❌ TypeError: unhashable type: 'list'

# 这是理解 Python 对象模型的重要概念，但日常使用中只需记住：“纯元组能做键，含列表的不能”。
...
lst = [1, 2, 3]
tup = (1, 2, 3)
print(sys.getsizeof(lst)) # 例如：88 字节
print(sys.getsizeof(tup)) # 例如：64 字节（更小！）
